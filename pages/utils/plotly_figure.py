...
def RSI(dataframe, num_period):
    dataframe['RSI'] = RSIIndicator(close=dataframe['Close'], window=14).rsi()
    dataframe = filter_data(dataframe,num_period)
    fig = go.Figure()
...
def Moving_average(dataframe,num_period):
    ...
    dataframe['SMA_50'] = SMAIndicator(close=dataframe['Close'], window=50).sma_indicator()
    dataframe = filter_data(dataframe,num_period)
    fig = go.Figure()
...
def MACD(dataframe, num_period):
    macd_indicator = MACDIndicator(close=dataframe['Close'])
